...
func NewDoubleHashTable(capacity int) *DoubleHashTable {
	...
	hashTable := DoubleHashTable{capacity: capacity, employees: employees}
	return &hashTable
}
...
func (hashTable *DoubleHashTable) find(name string) (int, int) {
	hash1 := hash(name) % hashTable.capacity
	hash2 := hash2(name) % hashTable.capacity
	index := 0
	deletedIndex := -1
	psl := 0
	for psl < hashTable.capacity {
		index = (hash1 + psl*hash2) % hashTable.capacity // Double hashing.
		if hashTable.employees[index] == nil {
			if deletedIndex >= 0 {
				return deletedIndex, psl + 1
			} else {
				return index, psl + 1
			}
